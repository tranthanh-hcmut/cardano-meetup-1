 ...
 * ```
 * */
const deriveAddress = (accountPublicKey, role, addressIndex, network) => {
    ...
    const networkInfo = {
        mainnet: cardano_serialization_lib_nodejs_1.NetworkInfo.mainnet(),
        preprod: cardano_serialization_lib_nodejs_1.NetworkInfo.testnet_preprod(),
        preview: cardano_serialization_lib_nodejs_1.NetworkInfo.testnet_preview(),
    };
    ...
    const utxoStakeCred = cardano_serialization_lib_nodejs_1.Credential.from_keyhash(utxoPubKeyHash);
    const mainStakeCred = cardano_serialization_lib_nodejs_1.Credential.from_keyhash(mainStakeKeyHash);
    ...
    if (role === 2) {
        ...
        const stakeCred = cardano_serialization_lib_nodejs_1.Credential.from_keyhash(stakeKeyHash);
